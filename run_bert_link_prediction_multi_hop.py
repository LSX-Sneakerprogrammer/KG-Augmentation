                ...
                if rnd <= 0.5:
                    ...
                    for j in range(5):
                        ...
                        while True:
                            ...
                            for t in range(1, len_line - 1):
                                tmp_triple_str += line[t]
                                tmp_triple_str += '\t'
                            tmp_triple_str += line[-1]
                            if tmp_triple_str not in lines_str_set:
                                break                    
                        tmp_head_text = ent2text[tmp_head]
                        examples.append(
...
class BertForLinkPrediction(BertPreTrainedModel):
    ...
    def forward(
        ...
        pooled_output = nn.functional.normalize(pooled_output, dim=1)
...
def _truncate_seq_triple_multi_hop(tokens_a, tokens_b, tokens_c, max_length):
    ...
    while True:
        ...
        if total_length <= max_length:
            break
        if len(tokens_a) > max(len_tokens_b) and len(tokens_a) > len(tokens_c):
            tokens_a.pop()
        elif max(len_tokens_b) >= len(tokens_a) and max(len_tokens_b) >= len(tokens_c):
            max_i = len_tokens_b.index(max(len_tokens_b))
            tokens_b[max_i].pop()
        elif len(tokens_c) > len(tokens_a) and len(tokens_c) > max(len_tokens_b):
            tokens_c.pop()
        else:
            if not tokens_c:
                ...
                raise KeyError("There is no tokens_c")
            tokens_c.pop()
...
def main():
    ...
    parser.add_argument("--bert_model", default="/export2/liushixuan/EvalKABERT/ANU/kg-bert-master/bert-base-uncased", type=str, required=True,
    ...
    parser.add_argument("--output_dir",
                        default="link_prediction/umls_extend",
                        type=str,
                        required=True,
    ...
    # model = BertForLinkPrediction.from_pretrained(args.bert_model)
    if args.fp16:
        model.half()
