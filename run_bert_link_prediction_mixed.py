    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if example.text_b and example.text_c:
            if example.set_type == "train_extend":
                tokens_b = [tokenizer.tokenize(example.text_b[i]) for i in range(len(example.text_b))]
            else:
        ...
        if tokens_b:
            if example.set_type == "train_extend":
                for i in range(len(tokens_b)):
                    tokens += tokens_b[i] + ["[SEP]"]
                    segment_ids += [1] * (len(tokens_b[i]) + 1)
                    sim_drop_mask += [0] * (len(tokens_b[i]) + 1)
            else:
                tokens += tokens_b + ["[SEP]"]
...
class BertForSimilarDropout(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.dropout_for_sim = nn.Dropout(0.4)
